...
/* values larger or equal than nen1 are wells with infinite energy*/
  int i,j,k,l,nat2,unstable,ntot1;
...
int add_gaps(double gap)
{
  ...
  for(i=1;i<=nat;i++)
	...
	if(next>=nen1)
	  {
	    init_coll(num,-1,next,sam[i].m,sam[j].m,dblarg1,sqrt(coll[next].dd)*(1+gap),-1);
            icoll[i][j]=num;
	    icoll[j][i]=num;
            coll[next].prev=num;
            num++;
...
{  
  ...
  printf("random seed=?\n");
  scanf("%d",&rn);
  printf("what is neibor update radius=?\n");
  scanf("%lf",&eps);
	...
	case LIST_ATOMS:
	  {
	    int na=0;
	    printf("%s\n",keywords[current_key]);
	    set_new_bounds(L,maxrb+3*eps,ndim);
	    set_rneib(eps,maxrb);	  
	    dummy=top;
            do
	      {
